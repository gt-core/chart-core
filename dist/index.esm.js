...
function logTag() {
    log('%c❤️ Welcome to klinecharts. Version is 10.0.0-beta3', 'border-radius:4px;border:dashed 1px #1677FF;line-height:70px;padding:0 20px;margin:16px 0;font-size:14px;color:#1677FF;', '', '', '', '');
}
        ...
        if (this._prevPressedPoint !== null) {
            ...
            this.points = this._prevPressedPoints.map(function (p) {
                ...
                if (isNumber(difDataIndex_1) && (isNumber(p.dataIndex) || isNumber(p.timestamp))) {
                    var dataIndex = isNumber(p.timestamp)
                        ? _this.isContinuousDrawingMode()
                    ...
                    newPoint.timestamp = _this.isContinuousDrawingMode()
                        ? (_a = chartStore.floatIndexToTimestamp(newPoint.dataIndex)) !== null && _a !== void 0 ? _a : undefined
                        : (_b = chartStore.dataIndexToTimestamp(newPoint.dataIndex)) !== null && _b !== void 0 ? _b : undefined;
                }
...
var StoreImp = /** @class */ (function () {
    function StoreImp(chart, options) {
        ...
        * Whether there are forward and backward more flag
        ...
        this._layoutBasicParams = __assign({}, DEFAULT_LAYOUT_BASIC_PARAMS);
        /**
    ...
    StoreImp.prototype.setHotkey = function (hotkey) { merge(this._hotKey, hotkey); };
    StoreImp.prototype.getHotkey = function () { return this._hotKey; };
    StoreImp.prototype.getHotKey = function () { return this._hotKey; };
    ...
    StoreImp.prototype.setBarSpace = function (barSpace, adjustBeforeFunc) {
        if (barSpace < this._layoutBasicParams.barSpaceLimitMin ||
    ...
    StoreImp.prototype.getLayoutBasicParams = function () {
                                ...
                                switch (figure.type) {
                                    case 'text': {
                                        attrs = __assign({ x: x, 
                                            ...
                                            y: valueY, 
                                            // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment -- ignore
                                            text: currentData === null || currentData === void 0 ? void 0 : currentData[figure.key], align: 'center', baseline: 'middle' }, attrs);
                                        break;
                                    }
                                    case 'circle': {
                                        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment -- ignore
...
var IndicatorLastValueView = /** @class */ (function (_super) {
    ...
    IndicatorLastValueView.prototype.drawImp = function (ctx) {
        ...
        if (lastValueMarkStyles.show) {
            ...
            var indicators = chartStore.getIndicatorsByPaneId(pane.getId()).filter(function (indicator) { return indicator.yAxisId === yAxis_1.id; });
...
var CrosshairHorizontalLabelView = /** @class */ (function (_super) {
        ...
        if (isString(crosshair.paneId) && this.compare(crosshair, pane.getId())) {
            ...
            if (styles.show) {
                ...
                if (directionStyles.show && textStyles.show) {
                    var bounding = widget.getBounding();
                    var axis = 'getAxisComponent' in widget
                        ? widget.getAxisComponent()
    ...
    CrosshairHorizontalLabelView.prototype.getText = function (crosshair, chartStore, axis) {
        ...
        if (yAxis.isInCandle() && yAxis.id === DEFAULT_AXIS_ID) {
            ...
            var indicators = chartStore.getIndicatorsByPaneId(crosshair.paneId).filter(function (indicator) { return indicator.yAxisId === yAxis.id; });
            indicators.forEach(function (indicator) {
...
var YAxisImp = /** @class */ (function (_super) {
    ...
    YAxisImp.prototype.createRangeImp = function () {
        ...
        var indicators = chartStore.getIndicatorsByPaneId(paneId).filter(function (indicator) { return indicator.yAxisId === _this.id; });
        indicators.forEach(function (indicator) {
        ...
        var isDefaultYAxis = this.id === DEFAULT_AXIS_ID;
        if (inCandle) {
        ...
        var shouldCompareHighLow = (inCandle && isDefaultYAxis && !isArea) || (!inCandle && shouldOhlc);
    ...
    YAxisImp.prototype.createTicksImp = function () {
        ...
        var chartStore = pane.getChart().getChartStore();
        var optimalTicks = [];
        var indicators = chartStore.getIndicatorsByPaneId(pane.getId()).filter(function (indicator) { return indicator.yAxisId === _this.id; });
        var styles = chartStore.getStyles();
        var precision = 0;
        var shouldFormatBigNumber = false;
...
var DrawPane = /** @class */ (function (_super) {
    ...
    DrawPane.prototype.createYAxis = function (axis) {
        var _a, _b, _c, _d;
        var yAxisId = (_a = axis.id) !== null && _a !== void 0 ? _a : DEFAULT_AXIS_ID;
        var yAxisName = (_b = axis.name) !== null && _b !== void 0 ? _b : 'normal';
        var needWidget = (_c = axis.needWidget) !== null && _c !== void 0 ? _c : true;
        ...
        if (shouldCreateYAxis) {
            (_d = this._yAxisWidgets.get(yAxisId)) === null || _d === void 0 ? void 0 : _d.destroy();
    ...
    DrawPane.prototype._getDefaultYAxisId = function () {
        var _a;
        if (this._yAxisComponents.has(DEFAULT_AXIS_ID)) {
            return DEFAULT_AXIS_ID;
        }
        // eslint-disable-next-line @typescript-eslint/no-unsafe-return -- ignore
        return (_a = this._yAxisComponents.keys().next().value) !== null && _a !== void 0 ? _a : null;
    };
    DrawPane.prototype.getYAxisComponentById = function (yAxisId) {
        var id = yAxisId !== null && yAxisId !== void 0 ? yAxisId : this._getDefaultYAxisId();
    ...
    DrawPane.prototype.getYAxisWidget = function () { return this.getYAxisWidgetById(); };
    DrawPane.prototype.getYAxisWidgets = function () { return Array.from(this._yAxisWidgets.values()); };
    DrawPane.prototype.updateImp = function (level) {
        this._mainWidget.update(level);
        this._yAxisWidgets.forEach(function (widget) { widget.update(level); });
    };
    DrawPane.prototype.destroy = function () {
        this._mainWidget.destroy();
        this._yAxisWidgets.forEach(function (widget) { widget.destroy(); });
    };
    ...
    DrawPane.prototype.createYAxisWidget = function (_container, _yAxis) { return null; };
    return DrawPane;
}(Pane));
...
var Event = /** @class */ (function () {
    ...
    Event.prototype._processYAxisScalingEvent = function (widget, event) {
        ...
        if (!consumed) {
            var yAxis = this._getYAxisByWidget(widget);
            var prevYAxisRange = this._prevYAxisRanges.get(yAxis);
...
var ChartImp = /** @class */ (function () {
    function ChartImp(container, options) {
        ...
        this._scheduleResize = function () {
            if (_this._resizeRequestAnimationId === DEFAULT_REQUEST_ID) {
                _this._resizeRequestAnimationId = requestAnimationFrame(function () {
                    ...
                    if (_this._chartBounding.width !== Math.floor(_this._chartContainer.clientWidth) ||
                        _this._chartBounding.height !== Math.floor(_this._chartContainer.clientHeight)) {
                        _this.resize();
                    }
                });
            }
        ...
        var defaultPaneOptions = this._getLayoutDefaultPaneOptions(this._chartStore.getLayoutBasicParams());
        var defaultYAxis = this._getLayoutDefaultYAxis(this._chartStore.getLayoutBasicParams());
        this._candlePane = this._createPane(CandlePane, __assign(__assign({}, defaultPaneOptions), { id: PaneIdConstants.CANDLE }));
        this._candlePane.createYAxis(__assign(__assign({}, defaultYAxis), { id: DEFAULT_AXIS_ID }));
        this._xAxisPane = this._createPane(XAxisPane, __assign(__assign({}, defaultPaneOptions), { id: PaneIdConstants.X_AXIS, order: Number.MAX_SAFE_INTEGER }));
        this._applyLayout(options === null || options === void 0 ? void 0 : options.layout);
        this._layout();
    ...
    ChartImp.prototype._getLayoutDefaultPaneOptions = function (basicParams) {
        var options = {};
    ...
    ChartImp.prototype._getLayoutDefaultYAxis = function (basicParams) {
        var yAxis = {};
    ...
    ChartImp.prototype._createLayoutIndicator = function (paneId, content, paneOptions, yAxis) {
        var _a;
        var indicator = '';
        var contentYAxis = null;
        if (isString(content)) {
            indicator = content;
        }
            ...
            contentYAxis = (_a = child.yAxis) !== null && _a !== void 0 ? _a : null;
        }
        else {
            indicator = content;
        }
        this.createIndicator(indicator, {
            isStack: true,
            pane: __assign(__assign({}, paneOptions), { id: paneId }),
            yAxis: __assign(__assign({}, yAxis), contentYAxis)
        });
    };
    ChartImp.prototype._applyLayout = function (layout) {
        var _this = this;
        var _a;
        if (!isValid(layout)) {
            return;
        }
        var basicParams = this._chartStore.getLayoutBasicParams();
        var defaultPaneOptions = this._getLayoutDefaultPaneOptions(basicParams);
        var defaultYAxis = this._getLayoutDefaultYAxis(basicParams);
        var panes = (_a = layout.panes) !== null && _a !== void 0 ? _a : [];
        panes.forEach(function (pane, index) {
            var _a, _b, _c, _d;
            var paneOptions = __assign(__assign({}, defaultPaneOptions), pane.options);
            switch (pane.type) {
                case 'candle': {
                    _this._candlePane.setOptions(__assign(__assign({}, paneOptions), { id: PaneIdConstants.CANDLE }));
                    _this._candlePane.createYAxis(__assign(__assign({}, defaultYAxis), { id: DEFAULT_AXIS_ID, paneId: PaneIdConstants.CANDLE }));
                    (_a = pane.content) === null || _a === void 0 ? void 0 : _a.forEach(function (content) {
                        _this._createLayoutIndicator(PaneIdConstants.CANDLE, content, __assign(__assign({}, paneOptions), { id: PaneIdConstants.CANDLE }), defaultYAxis);
                    });
                    break;
                }
                case 'indicator': {
                    var paneId_1 = (_b = paneOptions.id) !== null && _b !== void 0 ? _b : createId(PaneIdConstants.INDICATOR);
                    var currentPane = _this.getDrawPaneById(paneId_1);
                    if (!isValid(currentPane)) {
                        currentPane = _this._createPane(IndicatorPane, __assign(__assign({}, paneOptions), { id: paneId_1, order: (_c = paneOptions.order) !== null && _c !== void 0 ? _c : index + 1 }));
                    }
                    else {
                        currentPane.setOptions(__assign(__assign({}, paneOptions), { id: paneId_1 }));
                    }
                    currentPane.createYAxis(__assign(__assign({}, defaultYAxis), { id: DEFAULT_AXIS_ID, paneId: paneId_1 }));
                    (_d = pane.content) === null || _d === void 0 ? void 0 : _d.forEach(function (content) {
                        _this._createLayoutIndicator(paneId_1, content, __assign(__assign({}, paneOptions), { id: paneId_1 }), defaultYAxis);
                    });
                    break;
                }
                case 'xAxis': {
                    _this._xAxisPane.setOptions(__assign(__assign({}, paneOptions), { id: PaneIdConstants.X_AXIS }));
                    break;
                }
            }
        });
        ...
        if (measureHeight) {
            ...
            if (isValid(maximizedPane_1)) {
                ...
                contentPanes.forEach(function (pane) {
                    ...
                    var paneHeight = PANE_MIN_HEIGHT;
        ...
        var buildYAxisTickAndMeasureWidth = function () {
            ...
            if (forceMeasureWidth) {
                ...
                _this._drawPanes.forEach(function (pane) {
                    ...
                    leftOutsideAxes.forEach(function (yAxis, index) { updateColumnWidth_1(leftOutsideYAxisWidths_1, index, yAxis.getAutoSize()); });
                    leftInsideAxes.forEach(function (yAxis, index) { updateColumnWidth_1(leftInsideYAxisWidths_1, index, yAxis.getAutoSize()); });
                    rightInsideAxes.forEach(function (yAxis, index) { updateColumnWidth_1(rightInsideYAxisWidths_1, index, yAxis.getAutoSize()); });
                    rightOutsideAxes.forEach(function (yAxis, index) { updateColumnWidth_1(rightOutsideYAxisWidths_1, index, yAxis.getAutoSize()); });
                });
    ...
    ChartImp.prototype.getHotkey = function () { return this._chartStore.getHotkey(); };
    ChartImp.prototype.getHotKey = function () { return this._chartStore.getHotKey(); };
    ...
    ChartImp.prototype._syncIndicatorPanesByData = function () {
        var _this = this;
        var changed = false;
        var usedPaneIds = new Set([PaneIdConstants.CANDLE, PaneIdConstants.X_AXIS]);
        var defaultPaneOptions = this._getLayoutDefaultPaneOptions(this._chartStore.getLayoutBasicParams());
        this._chartStore.getIndicatorsByFilter({}).forEach(function (indicator) {
            usedPaneIds.add(indicator.paneId);
            if (!isValid(_this.getDrawPaneById(indicator.paneId))) {
                _this._createPane(IndicatorPane, __assign(__assign({}, defaultPaneOptions), { id: indicator.paneId }));
                changed = true;
            }
        });
        var removePaneIds = [];
        ...
        removePaneIds.forEach(function (paneId) {
    ...
    ChartImp.prototype._syncYAxesByData = function () {
        var _this = this;
        var changed = false;
        var defaultYAxis = this._getLayoutDefaultYAxis(this._chartStore.getLayoutBasicParams());
        this._drawPanes.forEach(function (pane) {
            ...
            _this._chartStore.getIndicatorsByPaneId(paneId).forEach(function (indicator) {
                ...
                if (!pane.hasYAxisComponent(indicator.yAxisId)) {
                    pane.createYAxis(__assign(__assign({}, defaultYAxis), { id: indicator.yAxisId, paneId: paneId }));
                    changed = true;
                }
            });
    ...
    ChartImp.prototype.createIndicator = function (value, options) {
        var _a, _b, _c, _d, _e;
        ...
        var paneId = (_b = (_a = options === null || options === void 0 ? void 0 : options.pane) === null || _a === void 0 ? void 0 : _a.id) !== null && _b !== void 0 ? _b : createId(PaneIdConstants.INDICATOR);
        var yAxisId = (_d = (_c = options === null || options === void 0 ? void 0 : options.yAxis) === null || _c === void 0 ? void 0 : _c.id) !== null && _d !== void 0 ? _d : DEFAULT_AXIS_ID;
        indicator.paneId = paneId;
        indicator.yAxisId = yAxisId;
        if (!isString(indicator.id)) {
            indicator.id = createId(indicator.name);
        }
        var result = this._chartStore.addIndicator(indicator, (_e = options === null || options === void 0 ? void 0 : options.isStack) !== null && _e !== void 0 ? _e : false);
        if (result) {
            ...
            var defaultYAxis = this._getLayoutDefaultYAxis(this._chartStore.getLayoutBasicParams());
            if (!isValid(pane)) {
                pane = this._createPane(IndicatorPane, __assign(__assign(__assign({}, defaultPaneOptions), options === null || options === void 0 ? void 0 : options.pane), { id: paneId }));
                shouldSort = true;
            }
            else if (isValid(options === null || options === void 0 ? void 0 : options.pane)) {
                pane.setOptions(__assign(__assign({}, options.pane), { id: paneId }));
                shouldSort = isNumber(options.pane.order);
            }
            pane.createYAxis(__assign(__assign(__assign({}, defaultYAxis), options === null || options === void 0 ? void 0 : options.yAxis), { id: yAxisId, paneId: paneId }));
    ...
    ChartImp.prototype.overrideIndicator = function (override) {
        var indicators = this._chartStore.getIndicatorsByFilter(override);
        if (indicators.length === 0) {
            return false;
        }
    ...
    ChartImp.prototype.removeIndicator = function (filter) {
        ...
        if (removed) {
            var panesChanged = this._syncIndicatorPanesByData();
            this._syncYAxesByData();
            this.layout({
                sort: panesChanged,
    ...
    ChartImp.prototype.overrideYAxis = function (yAxis) {
        var e_2, _a;
        var validPaneId = isValid(yAxis.paneId);
        var shouldLayout = false;
        try {
            for (var _b = __values(this._drawPanes), _c = _b.next(); !_c.done; _c = _b.next()) {
                var currentPane = _c.value;
                var currentPaneId = currentPane.getId();
